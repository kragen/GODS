...
#define __GDECLARE_CORE_ALLOC_FUNCTIONS( __structName, Token)												\
	void			gcreateData( __structName** inout_p##Token );											\
	void			gfreeData( __structName** inout_p##Token );												\
	void			gfreeData( __structName** inout_p##Token, uint32_t* n##Token##Count );					\
	__structName*	gacquireData( __structName* in_##Token );												\
	void			gcopyData( __structName* dst_##Token, const __structName* src_##Token );				\
	void			gcloneData( __structName** out_p##Token, const __structName* src_##Token );				\
	int32_t			gcompareData( const __structName* in_##Token##A, const __structName* in_##Token##B );	\
	bool			gwritableData( const __structName* in_##Token );										\

// This macro is used to declare read/write functions of the structure data (serialization)
#define __GDECLARE_CORE_SERIALIZATION_FUNCTIONS(__structName, Token)																\
	uint32_t fileDeserializeData( __structName** out_lstDataInstances, uint32_t in_nInstanceCount, FILE* in_fp );					\
	uint32_t fileSerializeData( __structName* const* in_lstDataInstances, uint32_t in_nInstanceCount, FILE* out_fp );				\
	uint32_t memDeserializeData( __structName** out_lstDataInstances, uint32_t in_nInstanceCount, const void* in_pMemoryBuffer );	\
	uint32_t memSerializeData( __structName* const* in_lstDataInstances, uint32_t in_nInstanceCount, void* out_pMemoryBuffer );		\

#define __GDECLARE_CORE_STREAMING_FUNCTIONS( __structName, Token)																				\
	uint32_t fileReadData( __structName** out_lstDataInstances, uint32_t in_nInstanceCount, FILE* in_fp );										\
	uint32_t fileWriteData( __structName* const* in_lstDataInstances, uint32_t in_nInstanceCount, FILE* out_fp, __structName* DefaultData );	\
	uint32_t memReadData( __structName** out_lstDataInstances, uint32_t in_nInstanceCount, const void* in_pMemoryBuffer );						\
	uint32_t memWriteData( __structName*  const* in_lstDataInstances, uint32_t in_nInstanceCount, void* out_pMemoryBuffer, __structName* DefaultData );
...
#define __GDECLARE_CORE_DEBUG_STRING_FUNCTIONS(__structName, Token)											\
	void printInfoString( const __structName* in_##Token );													\
	void getInfoString( wchar_t* dst_pOutputBuffer, uint32_t nBufferSize, const __structName* in_##Token );	\
	void getInfoString( char* dst_pOutputBuffer, uint32_t nBufferSize, const __structName* in_##Token );
...
// This macro is used to declare factory/refcount functions for the structures that implement them
#define __GDECLARE_CORE_INTERFACE_FUNCTIONS(__structName, Token)						\
